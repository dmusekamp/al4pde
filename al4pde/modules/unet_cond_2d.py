# Licensed under the MIT license. Adapted from PDEArena https://github.com/pdearena/pdearena.
...
from pdearena.modules.conditioned.fourier_cond import SpectralConv2d
...
class Unet2D(nn.Module):
    ...
    def __init__(
        ...
        if use1x1:
            self.image_proj = nn.Conv2d(insize, n_channels, kernel_size=1, padding_mode=padding_mode)
        else:
        ...
        if use1x1:
            self.final = zero_module(nn.Conv2d(in_channels, out_channels, kernel_size=1, padding_mode=padding_mode))
        else:
            self.final = zero_module(nn.Conv2d(in_channels, out_channels, kernel_size=(3, 3),
