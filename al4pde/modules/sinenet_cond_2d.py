...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, in0_channels, cond_channels, padding_mode, num_blocks, residual, pool, avg, num_groups=1, norm: bool = True, activation="gelu", first=False, disentangle=True, use_scale_shift_norm: bool = False) -> None:
        ...
        if pool:
            ...
            raise NotImplementedError(f"Pool not implemented for Downsampling")
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, cond_channels, mult, padding_mode, num_blocks, residual, interp, interp_mode, num_groups=1, norm: bool = True, activation="gelu", use_scale_shift_norm: bool = False) -> None:
        ...
        if interp:
            ...
            self.up = CircularInterpolate(mode=interp_mode) if padding_mode == "circular" else partial(torch.nn.functional.interpolate, scale_factor=2, mode=interp_mode)
        else:
            raise NotImplementedError(f"Interpolation not implemented for Upsampling")
...
class SineNet(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        print(f"SINENET padding_mode: {padding_mode}")
        ...
        if self.activation is None:
            raise NotImplementedError(f"Activation function, {activation}, not yet implemented")
        ...
        # we do not use this in our active learning experiments
        ...
        # embed pde parameter values (eta: η), as many as we have in the pde, as feature vectors
        if self.param_conditioning is not None:
            if self.param_conditioning.startswith("scalar"):
                ...
                raise NotImplementedError(f"Param conditioning type, {self.param_conditioning}, not yet implemented")
        ...
        down_args = dict(cond_channels=time_embed_dim, norm=norm, activation=activation, residual=residual,
                         padding_mode=padding_mode, num_blocks=num_blocks, disentangle=disentangle, pool=down_pool,
                         avg=avg_pool, in0_channels=n_channels, use_scale_shift_norm=use_scale_shift_norm)
        up_args = dict(cond_channels=time_embed_dim, norm=norm, activation=activation, mult=mult, residual=residual,
                       padding_mode=padding_mode, num_blocks=num_blocks, interp=up_interpolation,
                       interp_mode=interpolation_mode, use_scale_shift_norm=use_scale_shift_norm)
        ...
        print(f"# param count: {par_ct}, M={mult}" + (f", diff: {par1 - par_ct}" if par1 else ""))
        print("Channels: " + "->".join([str(ch) for ch in
                                        [n_channels, int(n_channels * mult), int(n_channels * mult ** 2),
                                         int(n_channels * mult ** 3), int(n_channels * mult ** 4)]]))

    def forward(self, x, time=None, z=None):
        assert x.dim() == 5
        orig_shape = x.shape     # [nb, nt, nc, nx, ny]
        x = x.reshape(x.size(0), -1, *x.shape[3:])   # collapse/flatten T,C
        ...
        emb = 0   # for testing cases without both time and parameter conditioning; unconditioned setting
        # Disabling ΔT-embed for now since we do not use it currently.
        # if time is not None:
        ...
        if z is not None:
            if self.param_conditioning.startswith("scalar"):
                if z.ndim == 1:
                    z = z[:, None]        # promote to a column vector
