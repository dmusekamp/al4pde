...
from al4pde.tasks.sim.mp_pde_solvers.temporal.tableaux import *
...
class ERKSolver(Iterator):
    ...
    def __init__(self, tableau: Tableau, device: torch.cuda.device="cpu", conserve=False) -> None:
        ...
        self.tableau = tableau
        self.device = device
        self.a = torch.from_numpy(tableau.a).to(self.device)
        self.b = torch.from_numpy(tableau.b).to(self.device)
        self.c = torch.from_numpy(tableau.c).to(self.device)
        self.s = tableau.s
        assert tableau.is_explicit, "Explicit tableau expected"
        self.conserve = conserve
        self.adaptive = tableau.is_adaptive
        if self.adaptive:
            self.blo = torch.from_numpy(tableau.blo).to(self.device)
            self.atol = torch.tensor(tableau.atol, dtype=torch.float64).to(self.device)
            self.rtol = torch.tensor(tableau.rtol, dtype=torch.float64).to(self.device)
